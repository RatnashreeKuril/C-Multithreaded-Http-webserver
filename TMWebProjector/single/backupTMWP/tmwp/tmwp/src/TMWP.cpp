...
using namespace std;
bool Comparator::operator()(string *left,string *rigth)
{
return (*left)<(*rigth);
}
...
d=new Data;
string *k=new string(iKey);
d->key=iKey;
d->value=iValue;
dataMap.insert(pair<string *, Data *>(k,d));
}
j++;
}
}
...
Data *d;
char *key;
map<string *,Data *, Comparator>::iterator i;
...
TMWebProjector::UrlData::UrlData()
{
this->url=NULL;
...
string *k=new string(d->url);
this->dataModel.urlAndPtrOnRequestMap.insert(pair<string *,UrlData *>(k,d));
}
...
{
map<string *, UrlData *,Comparator>::iterator i;
string k(request->resource);
i=dataModel.urlAndPtrOnRequestMap.find(&k);
